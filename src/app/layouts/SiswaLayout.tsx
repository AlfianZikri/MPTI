import { useState, useEffect } from "react";
import { Outlet } from "react-router";
import { Menu, LayoutDashboard, BookOpen, Calendar, ClipboardCheck } from "lucide-react";
import { Sidebar } from "../components/Sidebar";
import { supabase, type Siswa } from "../../lib/supabase";

const DEMO_NISN = "0123456789";

const navItems = [
  { label: "Dashboard", href: "/siswa", icon: LayoutDashboard },
  { label: "Nilai Saya", href: "/siswa/nilai", icon: BookOpen },
  { label: "Jadwal Pelajaran", href: "/siswa/jadwal", icon: Calendar },
  { label: "Absensi", href: "/siswa/absensi", icon: ClipboardCheck },
];

export function SiswaLayout() {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [siswa, setSiswa] = useState<Siswa | null>(null);

  useEffect(() => {
    supabase.from("siswa").select("*").eq("nisn", DEMO_NISN).single().then(({ data }) => {
      if (data) setSiswa(data as Siswa);
    });
  }, []);

  const initials = siswa ? siswa.nama.split(" ").map((w: string) => w[0]).join("").slice(0, 2).toUpperCase() : "AR";
  const title = siswa ? siswa.nama.split(" ").slice(0, 3).join(" ") : "Ahmad Rizki P.";
  const subtitle = siswa ? `Kelas ${siswa.kelas} · NISN ${siswa.nisn}` : "Kelas VI A";

  return (
    <div className="flex h-screen bg-background overflow-hidden">
      <Sidebar
        title={title}
        subtitle={subtitle}
        avatarInitials={initials}
        avatarColor="#2D6A4F"
        items={navItems}
        open={sidebarOpen}
        onClose={() => setSidebarOpen(false)}
      />
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-14 bg-card border-b border-border flex items-center px-6 gap-4 flex-shrink-0">
          <button className="lg:hidden text-muted-foreground hover:text-foreground" onClick={() => setSidebarOpen(true)}>
            <Menu size={20} />
          </button>
          <div className="ml-auto flex items-center gap-4">
            <span className="text-muted-foreground hidden sm:block" style={{ fontSize: "12px" }}>Semester Genap 2025/2026</span>
            <div className="w-8 h-8 flex items-center justify-center" style={{ backgroundColor: "#2D6A4F" }}>
              <span className="text-white" style={{ fontSize: "11px", fontWeight: 700 }}>{initials}</span>
            </div>
          </div>
        </header>
        <main className="flex-1 overflow-y-auto p-6 md:p-8">
          <Outlet />
        </main>
      </div>
    </div>
  );
}
